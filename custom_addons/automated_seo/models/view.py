...
class View(models.Model):
    ...
    def action_custom_button(self):
        ...
        html_parser = self.replace_php_tags_in_html(html_parser=html_parser)        # breakpoint()
        if html_parser:
    ...
    #     page_record = self.env['automated_seo.page'].search([('page_name', '=', view_name)], limit=1)
    #     website_page = self.env['website.page'].search([('name', '=', view_name)], limit=1)
    #     html_content = website_page.view_id.arch
    #
    #     soup = BeautifulSoup(html_content, "html.parser")
    #     sections = soup.find_all('section', {'data-snippet': True})
    #
    #     if not page_record:
    #         page_record = self.env['automated_seo.page'].create({'page_name': view_name})
    #         self._create_snippet_mappers(sections)
    #
    #     self._update_existing_snippet_ids(sections, website_page, soup)
    def update_snippet_ids(self, view_name):
        ...
        soup = BeautifulSoup(html_parser, "html.parser")
        # temp_soup = BeautifulSoup(html_parser, "html.parser")

        sections = soup.find_all('section', {'data-snippet': True})
        # temp_sections = soup.find_all('section', {'data-snippet': True})
        ...
        if not page:
            ...
            for section in sections:
                ...
                # temp_snippet_ids.append(section.get('data-snippet') + '-' + self.generate_hash())
    ...
    def replace_php_tags_in_html(self, html_parser):
        ...
        for section in sections:
            snippet_records = self.env['automated_seo.snippet_mapper'].search([('snippet_id', '=', section.get('data-snippet'))])
    ...
    # def _create_snippet_mappers(self, sections):
    #     snippet_ids = []
    #
    #     for section in sections:
    #         new_snippet_id = section.get('data-snippet') + '-' + self.generate_hash()
    #         snippet_ids.append(new_snippet_id)
    #         section['data-snippet'] = new_snippet_id
    #
    #         original_snippet_id = new_snippet_id.split('-')[0]
    ...
    #                 'php_tag': record.get('php_tag'),
    #                 'element_class': record.get('element_class'),
    #                 'image_name': record.get('image_name'),
    #             })
    #
    # def _update_existing_snippet_ids(self, sections, website_page, soup):
    #     for section in sections:
    #         current_snippet = section.get('data-snippet')
    #         if len(current_snippet.split('-')) != 2:
    #             new_snippet_id = current_snippet + '-' + self.generate_hash()
    #             section['data-snippet'] = new_snippet_id
    #
    #             original_snippet_id = new_snippet_id.split('-')[0]
    #             snippet_records = self.env['automated_seo.mapper'].search([('snippet_id', '=', original_snippet_id)],
    ...
    def upload_file_to_s3(self, file,view_name, s3_filename):
        s3 = boto3.client('s3',
